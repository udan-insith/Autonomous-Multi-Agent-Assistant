/**
 * dashboard.animations.ts
 * Every @trigger used by the dashboard, kept hardware-accelerated
 * (transform + opacity only) so they stay smooth under heavy
 * WebSocket-driven change detection.
 */
import {
  animate,
  animateChild,
  group,
  keyframes,
  query,
  stagger,
  state,
  style,
  transition,
  trigger,
} from '@angular/animations';

/**
 * agentStateAnimation
 * Drives the visual language of an agent card:
 *  - 'working'   -> infinite pulse + glow rotation
 *  - 'completed' -> checkmark slides down + settles
 *  - 'failed'    -> horizontal shake
 *  - 'idle'      -> resting state
 */
export const agentStateAnimation = trigger('agentStateAnimation', [
  state('idle', style({ transform: 'scale(1)', boxShadow: '0 0 0 rgba(16,185,129,0)' })),
  state('completed', style({ transform: 'scale(1)', boxShadow: '0 0 24px rgba(16,185,129,0.25)' })),
  state('failed', style({ transform: 'scale(1)', boxShadow: '0 0 24px rgba(244,63,94,0.25)' })),

  // Entering the 'working' state: quick settle, animation itself is
  // handled by a looping CSS class (see host binding) for perf, but we
  // also give a snappy scale kick on transition.
  transition('* => working', [
    animate(
      '350ms cubic-bezier(0.22, 1, 0.36, 1)',
      keyframes([
        style({ transform: 'scale(1)', offset: 0 }),
        style({ transform: 'scale(1.03)', offset: 0.5 }),
        style({ transform: 'scale(1.0)', offset: 1 }),
      ]),
    ),
  ]),

  transition('* => completed', [
    style({ boxShadow: '0 0 0 rgba(16,185,129,0)' }),
    animate('420ms cubic-bezier(0.22, 1, 0.36, 1)'),
  ]),

  // 'failed' shake — pure transform, no layout thrash.
  transition('* => failed', [
    animate(
      '520ms cubic-bezier(.36,.07,.19,.97)',
      keyframes([
        style({ transform: 'translate3d(0,0,0)', offset: 0 }),
        style({ transform: 'translate3d(-6px,0,0)', offset: 0.15 }),
        style({ transform: 'translate3d(5px,0,0)', offset: 0.3 }),
        style({ transform: 'translate3d(-4px,0,0)', offset: 0.45 }),
        style({ transform: 'translate3d(4px,0,0)', offset: 0.6 }),
        style({ transform: 'translate3d(-2px,0,0)', offset: 0.75 }),
        style({ transform: 'translate3d(2px,0,0)', offset: 0.9 }),
        style({ transform: 'translate3d(0,0,0)', offset: 1 }),
      ]),
    ),
  ]),
]);

/**
 * checkmarkAnimation
 * The green checkmark badge that slides down + fades in once an
 * agent card transitions to 'completed'.
 */
export const checkmarkAnimation = trigger('checkmarkAnimation', [
  transition(':enter', [
    style({ transform: 'translate3d(0,-12px,0) scale(0.6)', opacity: 0 }),
    animate(
      '380ms 60ms cubic-bezier(0.34, 1.56, 0.64, 1)',
      style({ transform: 'translate3d(0,0,0) scale(1)', opacity: 1 }),
    ),
  ]),
  transition(':leave', [
    animate('160ms ease-in', style({ transform: 'translate3d(0,-8px,0) scale(0.8)', opacity: 0 })),
  ]),
]);

/**
 * listAnimation
 * Stagger animation for the streaming terminal: each new AgentLog
 * slides up from the bottom and fades in as it lands in the array.
 */
export const listAnimation = trigger('listAnimation', [
  transition('* => *', [
    query(
      ':enter',
      [
        style({ transform: 'translate3d(0,14px,0)', opacity: 0 }),
        stagger('40ms', [
          animate(
            '320ms cubic-bezier(0.22, 1, 0.36, 1)',
            style({ transform: 'translate3d(0,0,0)', opacity: 1 }),
          ),
        ]),
      ],
      { optional: true },
    ),
  ]),
]);

/**
 * chunkRevealAnimation
 * Applied to each streamed synthesis chunk in the literature review
 * canvas so text "types itself in" as WebSocket chunks arrive.
 */
export const chunkRevealAnimation = trigger('chunkRevealAnimation', [
  transition(':enter', [
    style({ opacity: 0, transform: 'translate3d(0,4px,0)', filter: 'blur(2px)' }),
    animate(
      '420ms cubic-bezier(0.22, 1, 0.36, 1)',
      style({ opacity: 1, transform: 'translate3d(0,0,0)', filter: 'blur(0)' }),
    ),
  ]),
]);

/**
 * nodeDiscover
 * Citation graph nodes spring/bounce outward from the center scale
 * point the instant they're discovered by the Paper Fetcher agent.
 */
export const nodeDiscover = trigger('nodeDiscover', [
  transition(':enter', [
    style({ transform: 'scale(0)', opacity: 0 }),
    animate(
      '620ms cubic-bezier(0.34, 1.56, 0.64, 1)',
      keyframes([
        style({ transform: 'scale(0)', opacity: 0, offset: 0 }),
        style({ transform: 'scale(1.25)', opacity: 1, offset: 0.55 }),
        style({ transform: 'scale(0.92)', opacity: 1, offset: 0.8 }),
        style({ transform: 'scale(1)', opacity: 1, offset: 1 }),
      ]),
    ),
  ]),
]);

/**
 * edgeDraw
 * Citation edges fade + grow in behind their nodes.
 */
export const edgeDraw = trigger('edgeDraw', [
  transition(':enter', [
    style({ opacity: 0, strokeDashoffset: 120 }),
    animate('520ms 80ms ease-out', style({ opacity: 0.5, strokeDashoffset: 0 })),
  ]),
]);

/**
 * panelEnter
 * Subtle orchestrated entrance for the four main dashboard panels
 * on initial load, staggered as a group so the page reads as one
 * deliberate moment rather than four independent pops.
 */
export const panelEnter = trigger('panelEnter', [
  transition(':enter', [
    query(
      '.panel-block',
      [
        style({ opacity: 0, transform: 'translate3d(0,18px,0)' }),
        stagger('90ms', [
          animate(
            '520ms cubic-bezier(0.22, 1, 0.36, 1)',
            style({ opacity: 1, transform: 'translate3d(0,0,0)' }),
          ),
        ]),
      ],
      { optional: true },
    ),
  ]),
]);

/** All triggers bundled for convenient `animations: [...dashboardAnimations]` usage. */
export const dashboardAnimations = [
  agentStateAnimation,
  checkmarkAnimation,
  listAnimation,
  chunkRevealAnimation,
  nodeDiscover,
  edgeDraw,
  panelEnter,
];
